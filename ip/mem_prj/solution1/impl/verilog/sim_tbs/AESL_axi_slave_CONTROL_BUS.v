...
//------------------------Parameter----------------------
`define TV_IN_mask "./c.mem_hw.autotvin_mask.dat"
`define TV_IN_test_init_arr_V "./c.mem_hw.autotvin_test_init_arr_V.dat"
parameter ADDR_WIDTH = 11;
parameter DATA_WIDTH = 32;
parameter mask_DEPTH = 1;
reg [31 : 0] mask_OPERATE_DEPTH = 0;
parameter mask_c_bitwidth = 32;
parameter test_init_arr_V_DEPTH = 256;
...
parameter test_init_arr_V_data_in_addr = 1024;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_mask [mask_DEPTH - 1 : 0];
reg mask_write_data_finish;
...
//write mask reg
reg [31 : 0] write_mask_count = 0;
reg write_mask_run_flag = 0;
reg write_one_mask_data_done = 0;
...
assign TRAN_CONTROL_BUS_write_data_finish = 1 & mask_write_data_finish & test_init_arr_V_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        mask_write_data_finish <= 0;
        write_mask_run_flag <= 0; 
        write_mask_count = 0;
        count_operate_depth_by_bitwidth_and_depth (mask_c_bitwidth, mask_DEPTH, mask_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_mask_run_flag <= 1; 
            write_mask_count = 0;
        end
        if (write_one_mask_data_done === 1) begin
            write_mask_count = write_mask_count + 1;
            if (write_mask_count == mask_OPERATE_DEPTH) begin
                write_mask_run_flag <= 0; 
                mask_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_mask
    integer write_mask_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] mask_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = mask_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_mask_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write mask data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (mask_c_bitwidth < 32) begin
                        mask_data_tmp_reg = mem_mask[write_mask_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < mask_c_bitwidth) begin
                                mask_data_tmp_reg[j] = mem_mask[write_mask_count][i*32 + j];
                            end
                            else begin
                                mask_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (mask_data_in_addr + write_mask_count * four_byte_num * 4 + i * 4, mask_data_tmp_reg, write_mask_resp);
                end
                process_busy = 0;
                write_one_mask_data_done <= 1;
                @(posedge clk);
                write_one_mask_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_mask_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [mask_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (mask_c_bitwidth , factor);
  fp = $fopen(`TV_IN_mask ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_mask); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < mask_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
