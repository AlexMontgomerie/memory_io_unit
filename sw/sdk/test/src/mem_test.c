...
int init_dma_intr(int dev_num){
	...
	case 0:
		device_id = XPAR_AXI_DMA_0_DEVICE_ID;
		vec_mm2s_id = XPAR_FABRIC_AXI_DMA_0_MM2S_INTROUT_INTR;
		vec_s2mm_id = XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR;
		break;
#endif
	...
	case 1:
		device_id = XPAR_AXI_DMA_1_DEVICE_ID;
		vec_mm2s_id = XPAR_FABRIC_AXI_DMA_1_MM2S_INTROUT_INTR;
		vec_s2mm_id = XPAR_FABRIC_AXI_DMA_1_S2MM_INTROUT_INTR;
		break;
#endif
	...
	case 2:
		device_id = XPAR_AXI_DMA_2_DEVICE_ID;
		vec_mm2s_id = XPAR_FABRIC_AXI_DMA_2_MM2S_INTROUT_INTR;
		vec_s2mm_id = XPAR_FABRIC_AXI_DMA_2_S2MM_INTROUT_INTR;
		break;
#endif
	...
	case 3:
		device_id = XPAR_AXI_DMA_3_DEVICE_ID;
		vec_mm2s_id = XPAR_FABRIC_AXI_DMA_3_MM2S_INTROUT_INTR;
		vec_s2mm_id = XPAR_FABRIC_AXI_DMA_3_S2MM_INTROUT_INTR;
		break;
#endif
	}
	...
	case 0:
		status = XScuGic_Connect(&ScuGic, XPAR_FABRIC_AXI_DMA_0_MM2S_INTROUT_INTR, (Xil_InterruptHandler)TxIntrHandler_0, &AxiDma[dev_num]);
		if (status != XST_SUCCESS) {
			return status;
		}

		status = XScuGic_Connect(&ScuGic, XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR, (Xil_InterruptHandler)RxIntrHandler_0, &AxiDma[dev_num]);
		if (status != XST_SUCCESS) {
			return status;
		}
		break;
	...
	case 1:
		status = XScuGic_Connect(&ScuGic, XPAR_FABRIC_AXI_DMA_1_MM2S_INTROUT_INTR, (Xil_InterruptHandler)TxIntrHandler_1, &AxiDma[dev_num]);
		if (status != XST_SUCCESS) {
			return status;
		}

		status = XScuGic_Connect(&ScuGic, XPAR_FABRIC_AXI_DMA_1_S2MM_INTROUT_INTR, (Xil_InterruptHandler)RxIntrHandler_1, &AxiDma[dev_num]);
		if (status != XST_SUCCESS) {
			return status;
		}
		break;
	...
	case 2:
		status = XScuGic_Connect(&ScuGic, XPAR_FABRIC_AXI_DMA_2_MM2S_INTROUT_INTR, (Xil_InterruptHandler)TxIntrHandler_2, &AxiDma[dev_num]);
		if (status != XST_SUCCESS) {
			return status;
		}

		status = XScuGic_Connect(&ScuGic, XPAR_FABRIC_AXI_DMA_2_S2MM_INTROUT_INTR, (Xil_InterruptHandler)RxIntrHandler_2, &AxiDma[dev_num]);
		if (status != XST_SUCCESS) {
			return status;
		}
		break;
	...
	case 3:
		status = XScuGic_Connect(&ScuGic, XPAR_FABRIC_AXI_DMA_3_MM2S_INTROUT_INTR, (Xil_InterruptHandler)TxIntrHandler_3, &AxiDma[dev_num]);
		if (status != XST_SUCCESS) {
			return status;
		}

		status = XScuGic_Connect(&ScuGic, XPAR_FABRIC_AXI_DMA_3_S2MM_INTROUT_INTR, (Xil_InterruptHandler)RxIntrHandler_3, &AxiDma[dev_num]);
		if (status != XST_SUCCESS) {
			return status;
		}
		break;
...
int RunDevice(int dev_num, data_t * in, data_t * out, int devAddr, int pageAddr, float res[3][2]){
	...
	for(i=0;i<index;i++) {
		data[i][2] = data[i][0]*data[i][1]*1000;
	}
...
int timing_test(data_t * in[NUM_HW], data_t * out[NUM_HW]) {
	int j;
	int time_val;
	for(j=0;j<NUM_HW;j++)
	{
